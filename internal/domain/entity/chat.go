...
type ChatConfig struct {
	Model             *Model
	Temperature       float32  // 0.0 to 1.0 presisão
	TopP              float32  // 0.0 to 1.0 presisão
...
func NewChat(userID string, initialSystemMessagem *Message, chatConfig *ChatConfig) (*Chat, error) {
	chat := &Chat {
		ID: uuid.New().String(),
		UserID: userID,
		InitialSystemMessage: initialSystemMessagem,
		Status: "active",
		Config: chatConfig,
		TokenUsage: 0,
	}
	chat.AddMessage(initialSystemMessagem)
